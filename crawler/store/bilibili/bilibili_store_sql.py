...
async def update_content_by_content_id(content_id: str, content_item: Dict) -> int:
    ...
    effect_row: int = await async_db_conn.update_table(
        "bilibili_video", content_item, "video_id", content_id
    )
    return effect_row
...
async def add_new_comment(comment_item: Dict) -> int:
    ...
    last_row_id: int = await async_db_conn.item_to_table(
        "bilibili_video_comment", comment_item
    )
    return last_row_id
...
async def update_comment_by_comment_id(comment_id: str, comment_item: Dict) -> int:
    ...
    async_db_conn: AsyncMysqlDB = media_crawler_db_var.get()
    effect_row: int = await async_db_conn.update_table(
        "bilibili_video_comment", comment_item, "comment_id", comment_id
    )
    return effect_row
...
async def update_creator_by_creator_id(creator_id: str, creator_item: Dict) -> int:
    ...
    async_db_conn: AsyncMysqlDB = media_crawler_db_var.get()
    effect_row: int = await async_db_conn.update_table(
        "bilibili_up_info", creator_item, "user_id", creator_id
    )
    return effect_row
