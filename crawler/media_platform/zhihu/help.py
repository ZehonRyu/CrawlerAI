...
# 使用与 core.py 相同的方法计算路径
current_file = os.path.abspath(__file__)
crawler_dir = os.path.dirname(os.path.dirname(os.path.dirname(current_file)))
ZHIHU_JS_PATH = os.path.join(crawler_dir, "libs", "zhihu.js")

# 添加调试信息
utils.logger.info(f"[DEBUG] __file__ path: {os.path.abspath(__file__)}")
utils.logger.info(f"[DEBUG] Crawler dir: {crawler_dir}")
utils.logger.info(f"[DEBUG] Zhihu JS path: {ZHIHU_JS_PATH}")
utils.logger.info(f"[DEBUG] Zhihu JS file exists: {os.path.exists(ZHIHU_JS_PATH)}")

# 检查文件是否存在
if not os.path.exists(ZHIHU_JS_PATH):
    raise FileNotFoundError(f"zhihu.js 文件未找到: {ZHIHU_JS_PATH}")
...
class ZhihuExtractor:
    ...
    def _extract_answer_content(self, answer: Dict) -> ZhihuContent:
        ...
        res.voteup_count = answer.get("voteCount", 0)
    ...
    def _extract_zvideo_content(self, zvideo: Dict) -> ZhihuContent:
        ...
        if "video" in zvideo and isinstance(
            zvideo.get("video"), dict
        ):  # 说明是从创作者主页的视频列表接口来的
    ...
    def _extract_content_or_comment_author(author: Dict) -> ZhihuCreator:
        ...
        try:
            ...
            res.user_link = (
                f"{zhihu_constant.ZHIHU_URL}/people/{author.get('url_token')}"
            )
