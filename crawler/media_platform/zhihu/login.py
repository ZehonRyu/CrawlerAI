...
class ZhiHuLogin(AbstractLogin):
    ...
    async def check_login_state(self) -> bool:
        ...
        try:
            current_cookie = await self.browser_context.cookies()
            if not current_cookie:  # 检查cookies是否为空
                utils.logger.warning(
                    "[ZhiHu.check_login_state] No cookies found in browser context"
                )
                return False
            ...
            # 确保类型安全
            return bool(current_web_session)  # 将任何非空值转为True，None转为False

        except Exception as e:
            utils.logger.error(
                f"[ZhiHu.check_login_state] Error checking login state: {str(e)}"
            )
            return False
    ...
    async def login_by_qrcode(self):
        ...
        utils.logger.info(
            f"[ZhiHu.login_by_qrcode] waiting for scan code login, remaining time is 120s"
        )
        try:
