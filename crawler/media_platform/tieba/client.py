...
class BaiduTieBaClient(AbstractApiClient):
    ...
    async def request(
        self, method, url, return_ori_content=False, proxies=None, **kwargs
    ) -> Union[str, Any]:
        """
    ...
    async def get(
        ...
        try:
            ...
            utils.logger.error(
                f"[BaiduTieBaClient.get] 达到了最大重试次数，IP已经被Block，请尝试更换新的IP代理: {e}"
            )
            raise Exception(
                f"[BaiduTieBaClient.get] 达到了最大重试次数，IP已经被Block，请尝试更换新的IP代理: {e}"
            )
    ...
    async def pong(self) -> bool:
        ...
        try:
            ...
            utils.logger.error(
                f"[BaiduTieBaClient.pong] Ping tieba failed: {e}, and try to login again..."
            )
            ping_flag = False
        return ping_flag
    ...
    async def get_comments_all_sub_comments(
        ...
        for parment_comment in comments:
            ...
            while max_sub_page_num >= current_page:
                ...
                page_content = await self.get(
                    uri, params=params, return_ori_content=True
                )
    ...
    async def get_creator_info_by_url(self, creator_url: str) -> str:
        ...
        """
        page_content = await self.request(
            method="GET", url=creator_url, return_ori_content=True
        )
        return page_content
