...
# 获取当前文件的绝对路径并计算项目相关路径
current_file = os.path.abspath(__file__)
# 计算 crawler 目录路径 (F:\Project\CrawlerAI\crawler)
crawler_dir = os.path.dirname(os.path.dirname(os.path.dirname(current_file)))
# 计算项目根目录 (F:\Project\CrawlerAI)
...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        # 添加调试信息
        utils.logger.info(f"[DEBUG] __file__ path: {os.path.abspath(__file__)}")
        utils.logger.info(f"[DEBUG] Crawler dir: {crawler_dir}")
        utils.logger.info(f"[DEBUG] Stealth path: {stealth_path}")
        utils.logger.info(f"[DEBUG] File exists: {os.path.exists(stealth_path)}")
        ...
        async with async_playwright() as playwright:
            ...
            elif config.CRAWLER_TYPE == "creator_audio":
                for creator_id in config.BILI_CREATOR_ID_LIST:
                    await self.get_creator_audio(int(creator_id))
            else:
                pass
    ...
    async def get_creator_videos(self, creator_id: int):
        ...
        while True:
            try:
                ...
                # 首次循环获取总视频数
                if pn == 1:
                    total_videos = int(result["page"]["count"])
                ...
                utils.logger.error(f"获取创作者视频失败: {e}")
                break
            except KeyError as e:
                utils.logger.error(f"API响应结构异常: {e}")
                break

        utils.logger.info(f"已获取 {len(video_bvids_list)}/{total_videos} 个视频")
        await self.get_specified_videos(video_bvids_list)
    ...
    async def get_creator_audio(self, creator_id: int):
        ...
        """
        # 1. 获取创作者视频列表（复用现有逻辑）
        video_bvids = await self.fetch_creator_videos(creator_id)

        # 2. 下载音频（不获取评论）
        await self.download_creator_audios(video_bvids)

    async def fetch_creator_videos(self, creator_id: int) -> List[str]:
        """复用现有视频获取逻辑（与get_creator_videos相同）"""
        ps = 30
        pn = 1
        video_bvids_list = []
        total_videos = 0
        ...
        while True:
            ...
            except (DataFetchError, KeyError) as e:
                utils.logger.error(f"获取创作者视频失败: {e}")
                break

        utils.logger.info(f"符合条件的视频数: {len(video_bvids_list)}/{total_videos}")
        print("video_bvids_list:", video_bvids_list)
        return video_bvids_list

    def should_download_video(self, video_info: Dict) -> bool:
        """视频过滤条件"""
        # 截止日期过滤
        if config.AUDIO_CUTOFF_DATE:
            publish_time = datetime.fromtimestamp(video_info["created"])
            if publish_time < config.AUDIO_CUTOFF_DATE:
                return False

        # 观看量过滤
        if config.MIN_VIEW_COUNT:
            if int(video_info.get("play", 0)) < config.MIN_VIEW_COUNT:
                return False

        # 点赞量过滤
        # if config.MIN_LIKE_COUNT:
        #     if int(video_info.get('like', 0)) < config.MIN_LIKE_COUNT:
        #         return False

        return True

    async def download_creator_audios(self, bvids_list: List[str]):
        """
        下载创作者视频的音频
        :param bvids_list: 视频BVID列表
        """
        semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
        task_list = [
            self.get_video_info_task(aid=0, bvid=video_id, semaphore=semaphore)
        ...
        video_details = await asyncio.gather(*task_list)
        # print(f"视频详情: {video_details}")
        audio_tasks = []
        for video_detail in video_details:
            if video_detail is not None:
                # 只下载音频，不获取评论
                audio_tasks.append(self.get_bilibili_audio(video_detail, semaphore))

        await asyncio.gather(*audio_tasks)

    async def get_bilibili_audio(self, video_item: Dict, semaphore: asyncio.Semaphore):
        """
        下载bilibili音频
        :param video_item: 视频详情
        :param semaphore: 并发控制
        :return:
        """
        # 检查是否启用音频下载
        if not config.ENABLE_GET_AUDIO:
            utils.logger.info(f"[BilibiliCrawler.get_bilibili_audio] 音频下载模式未启用")
            return
        ...
        # 获取播放信息
        play_info = await self.get_video_play_url_task(aid, cid, semaphore)
        if play_info is None:
            utils.logger.info("[BilibiliCrawler.get_bilibili_audio] 获取音频播放信息失败")
            return

        # 提取最佳音频URL
        audio_url = None
        if "dash" in play_info:  # 新版API返回dash格式
            audio_streams = play_info["dash"]["audio"]
            if audio_streams:
                # 选择最高质量的音频流
                best_audio = max(audio_streams, key=lambda x: x.get("id", 0))
                audio_url = (
                    best_audio.get("base_url") or best_audio.get("backup_url")[0]
                )

        # 旧版durl格式不支持纯音频下载
        if not audio_url:
            utils.logger.info("[BilibiliCrawler.get_bilibili_audio] 无法获取纯音频URL")
            return

        # 下载音频内容
        audio_content = await self.bili_client.get_video_media(audio_url)
        if audio_content is None:
            return

        # 保存音频文件
        extension_file_name = "audio.m4a"
        await bilibili_store.store_audio(aid, audio_content, extension_file_name)
