...
function i(e, t, n) {  // 将一个整数 e 按照字节顺序拆分为 4 个字节，并存储到数组 t 中，从索引 n 开始。（右移字节）
    t[n] = 255 & e >>> 24,
    t[n + 1] = 255 & e >>> 16,
    t[n + 2] = 255 & e >>> 8,
    t[n + 3] = 255 & e
...
function Q(e, t) {  // 对整数 e 进行循环移位操作，左移 t 位，同时右移 (32 - t) 位。
    return (4294967295 & e) << t | e >>> 32 - t
}

function B(e, t) {  // 从数组 e 中提取从索引 t 开始的 4 个字节，并将它们组合成一个整数。
    return (255 & e[t]) << 24 | (255 & e[t + 1]) << 16 | (255 & e[t + 2]) << 8 | 255 & e[t + 3]
}

function G(e) {  // 根据输入整数 e，通过一系列复杂的位运算和查表操作（使用 h.zb 数组），生成一个新的整数值。
    var t = new Array(4)
        , n = new Array(4);
    i(e, t, 0),
        n[0] = h.zb[255 & t[0]],
...
function array_0_16_offset(e) {  // 对长度为 16 的数组 e 进行加密处理，返回一个新的长度为 16 的数组。
    var t = new Array(16)
        , n = new Array(36);
    n[0] = B(e, 0),
        n[1] = B(e, 4),
...
function array_16_48_offset(e, t) {  // 对长度为 16 到 48 的数组 e 进行分块加密处理，返回一个新的数组。
...
function encode_0_16(array_0_16) {  // 对长度为 16 的数组进行 XOR 操作和位运算，返回加密后的数组。
    let result = [];
...
function encode(ar) {  // 将一个长度为 3 的数组编码为长度为 4 的数组，每个元素为 6 位的二进制值。
    let b = ar[1] << 8,
        c = ar[0] | b,
        d = ar[2] << 16,
        e = c | d,
...
function get_init_array(encode_md5) {  // 根据 MD5 值生成初始数组，长度为 48。
    let init_array = []
...
function get_zse_96(encode_md5) { // 根据 MD5 值生成 x-zse-96 签名值
    let result_array = [],
 ...
 * @returns {string}
 */
const extract_dc0_value_from_cookies = function (cookies) { // 从cookies中提取dc0的值
 ...
 * @returns {*}
 */
function get_sign(url, cookies) { // 核心函数，用于生成知乎请求的签名值。
    const dc0 = extract_dc0_value_from_cookies(cookies) // 从 Cookies 中提取 d_c0 值。
    const ta = "101_3_3.0" // 定义固定参数 ta 和 tc。
    ...
    const params_join_str = [ta, url, dc0, tc].join("+") // 拼接的字符串，包含固定参数、URL 和 d_c0 值，用于生成 MD5 值。
